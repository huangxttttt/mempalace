...
Run `uv tool list` or `uv pip show mempalace` to see if the package is already
present. If it is, report the installed version and skip to Step 4.
...
If `uv tool install mempalace` fails, try these fallbacks in order:

1. For local repo development, try `uv sync`
2. Try `uv pip install mempalace`
3. If you explicitly do not want `uv`, try `python -m pip install mempalace`
...
Run `uv run mempalace init <dir>` where `<dir>` is the directory from Step 4.
...
Run `uv run mempalace status` and confirm the output shows a healthy palace.
